...
void cQuanHam::LoadImageQuanHam()
{
	...
	{
		char fileName[100];
		sprintf(fileName, "TAS\\RankTitle\\QuanHam\\QuanHam_%d.tga", i);
		pLoadImage(fileName, TAS_QUANHAM_IMG + i , 0x2601, 0x2900, 1, 0);
	}
...
void cQuanHam::ShowQuanHamTAS(int index,int PosX, int PosY)
{
	this->TAS_QUANHAM_LVL = gCustomRankUser.m_Data[index].m_RankTitle2;
	if(this->TAS_QUANHAM_LVL >= 1 && this->TAS_QUANHAM_LVL <= MAX_LEVEL_QUAN_HAM )
	{
		glColor3f(1.0, 1.0, 1.0);
		RenderBitmap(TAS_QUANHAM_IMG + this->TAS_QUANHAM_LVL , (float)PosX + 16, (float)PosY - 70, 70, 56, 0, 0, 1, 1, 1, 1, 0.0);
	}	
}
...
void cQuanHam::DrawWindowNangCapQuanHam()
{
	...
	gCentral.gDrawWindowCustom(StartX, StartY, CuaSoW, CuaSoH, eWindow_NangCapQuanHam, gConfig.TTAS_TEXT_ENGINE[255]);
	gToolKit.CloseButton(512, 78, TAS_QuanHam_BtnClose);
	...
	{
		RenderBitmap(TAS_QUANHAM_IMG + LvQuanHam , StartX + 23, StartY + 25, 160, 156, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
		gInterface.TASText(eExcellent,StartX + 30, TextPlusY + 10, 125,				1, gConfig.TTAS_TEXT_ENGINE[220], (LvQuanHam == 0 || GetInfoLvHienTai == 0) ? 0 : GetInfoLvHienTai->TangMau);
		gInterface.TASText(eExcellent,StartX + 30, TextPlusY + 20, 125,				1, gConfig.TTAS_TEXT_ENGINE[221], (LvQuanHam == 0 || GetInfoLvHienTai == 0) ? 0 : GetInfoLvHienTai->TangSD);
		gInterface.TASText(eExcellent,StartX + 30, TextPlusY + (10 * 2) + 10, 125,  1, gConfig.TTAS_TEXT_ENGINE[222], (LvQuanHam == 0 || GetInfoLvHienTai == 0) ? 0 : GetInfoLvHienTai->TangST);
		gInterface.TASText(eExcellent,StartX + 30, TextPlusY + (10 * 3) + 10, 125,  1, gConfig.TTAS_TEXT_ENGINE[223], (LvQuanHam == 0 || GetInfoLvHienTai == 0) ? 0 : GetInfoLvHienTai->TangPT);

		if( LvQuanHam + 1 > MAX_LEVEL_QUAN_HAM )
		{
			RenderBitmap(TAS_QUANHAM_IMG, StartX + this->KhoanCach_Distance - 38, StartY + 25, 160, 156, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
		}
		else
		{
			RenderBitmap(TAS_QUANHAM_IMG + LvQuanHam + 1, StartX + this->KhoanCach_Distance - 38, StartY + 25, 160, 156, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
		}
		...
		CustomFont.Draw(CustomFont.FontBold,			StartX + this->KhoanCach_Distance - 40, StartY + 55 + 110, 0xFFDE26FF, 0x0, 125, 0, 3, gConfig.TTAS_TEXT_ENGINE[224], LvQuanHam + 1);
	}
	else
	{
		RenderBitmap(TAS_QUANHAM_IMG + LvQuanHam , StartX + 23, StartY + 25, 160, 156, 0.0, 0.0, 1.0, 1.0, 1, 1, 0.0);
	...
	CustomFont.Draw(CustomFont.FontBold, StartX + 23, StartY + 55 + 110, 0xFFDE26FF, 0x0, 125, 0, 3, gConfig.TTAS_TEXT_ENGINE[227], LvQuanHam);
	...
	{
		CustomFont.Draw(CustomFont.FontBold, StartX + 160 + 10, YUpCapQuanHam, 0xFFDE26FF, 0x0, CuaSoW - 170, 0, 1, gConfig.TTAS_TEXT_ENGINE[231], LvQuanHam + 1);
		...
		#else
		CustomFont.Draw(CustomFont.FontBold, StartX + 160 + 30, YUpCapQuanHam + 13, 0x1FD2FFFF, 0x0, CuaSoW - 140, 0, 1,
			"Cần: %d Viên %s",
			GetInfoLvTiepTheo->YCItemSL,
			ItemNameBankQH[GetInfoLvTiepTheo->YCItemID]);
		if (gCustomJewelBank.m_Data.ItemBank[GetInfoLvTiepTheo->YCItemID] < GetInfoLvTiepTheo->YCItemSL)
		{
			...
			if (KetQua)
			{
				CustomFont.Draw(CustomFont.FontBold, StartX + 160, YUpCapQuanHam + 13 + (12 * 5) + 5, 0x1FD2FFFF, 0x0, CuaSoW - 170, 0, 1, "Còn lại: %d viên %s => Thiếu %d viên", gCustomJewelBank.m_Data.ItemBank[GetInfoLvTiepTheo->YCItemID], ItemNameBankQH[GetInfoLvTiepTheo->YCItemID], KetQua);
			}
		}
		else
		{
			CustomFont.Draw(CustomFont.FontBold, StartX + 160, YUpCapQuanHam + 13 + (12 * 5) + 5, 0x1FD2FFFF, 0x0, CuaSoW - 170, 0, 1, "Còn lại: %d viên %s", gCustomJewelBank.m_Data.ItemBank[GetInfoLvTiepTheo->YCItemID], ItemNameBankQH[GetInfoLvTiepTheo->YCItemID]);
		}
		#endif
		gInterface.TASText(eExcellent, StartX + 160 + 30, YUpCapQuanHam + 13 + (12 * 1), 125,1,gConfig.TTAS_TEXT_ENGINE[233], GetInfoLvTiepTheo->YCWC);
		gInterface.TASText(eExcellent, StartX + 160 + 30, YUpCapQuanHam + 13 + (12 * 2), 125,1,gConfig.TTAS_TEXT_ENGINE[234], GetInfoLvTiepTheo->YCWP);
		gInterface.TASText(eExcellent, StartX + 160 + 30, YUpCapQuanHam + 13 + (12 * 3), 125,1,gConfig.TTAS_TEXT_ENGINE[235], GetInfoLvTiepTheo->YCGP);
		CustomFont.Draw(CustomFont.FontBold, StartX + 160, YUpCapQuanHam + 13 + (12 * 4) + 5, 0xFFDE26FF, 0x0, CuaSoW - 170, 0, 1, gConfig.TTAS_TEXT_ENGINE[236]);

		if (gCentral.gDrawButton((StartX + CuaSoW)/2 - 10, StartY + (CuaSoH - 40), 110, 14, gConfig.TTAS_TEXT_ENGINE[237]) && (GetTickCount() - gInterface.Data[eWindow_NangCapQuanHam].EventTick) > 300)
		{
	...
	{
		CustomFont.Draw(CustomFont.FontBold, StartX + 160, YUpCapQuanHam, 0xFFDE26FF, 0x0, CuaSoW - 170, 0, 1, gConfig.TTAS_TEXT_ENGINE[238]);
	}	
}
#endif
